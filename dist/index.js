...
async function getWorkflows(token, owner, repo, status) {
  const octokit = (0, import_github.getOctokit)(token);
  return octokit.paginate("GET /repos/{owner}/{repo}/actions/runs", { owner, repo, status }, (response) => response.data.map((w) => w.id));
}
async function deleteWorkflow(token, owner, repo, run_id) {
  const octokit = (0, import_github.getOctokit)(token);
  const result = await octokit.request("DELETE /repos/{owner}/{repo}/actions/runs/{run_id}", { owner, repo, run_id });
  if (result.status !== 204) {
    throw new Error("Failed to remove run " + run_id);
  }
}
...
function getRepositoryAndOwner(repository) {
...
function getWorkflowStatus(status) {
  if (VALID_STATUS.includes(status))
    return status;
  throw new Error(`Invalid status '${status}'. Expected value one of: ${VALID_STATUS.join(", ")}`);
}
function getInputs() {
  const repository = (0, import_core.getInput)("repository") || `${import_github2.context.repo.owner}/${import_github2.context.repo.repo}`;
  (0, import_core.debug)(`repository = '${repository}'`);
  const { owner, repo } = getRepositoryAndOwner(repository);
  const token = (0, import_core.getInput)("token", { required: true });
  const rawStatus = (0, import_core.getInput)("status") || "failure";
  const status = getWorkflowStatus(rawStatus);
  (0, import_core.debug)(`workflow status = '${status}'`);
...
async function run() {
  try {
    ...
    const run_ids = await getWorkflows(input.token, input.owner, input.repo, input.status);
    if (run_ids.length === 0) {
      (0, import_core2.info)(`No workflow with '${input.status}' found`);
      return;
    }
    for (let i = 0; i < run_ids.length; i += 1) {
      ...
      (0, import_core2.debug)(`workflow run ${run_id} deleted`);
    }
    (0, import_core2.info)(`Job done. ${run_ids.length} workflow with '${input.status}' removed`);
  } catch (error) {
    (0, import_core2.setFailed)(`${error?.message ?? error}`);
  }
}
run();
/*!
